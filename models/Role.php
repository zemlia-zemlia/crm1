...
class Role extends \yii\db\ActiveRecord
{
    ...
    {
           $list = self::find()->select(['name', 'description'])->asArray()->where(['type' => 1])->all();
        $list1 = [];
        foreach($list as $ls){
            if($ls['name'] != 'Client') $list1[] = $ls;
        }
        return ArrayHelper::map($list1,  'name','description' );
    }
